...
def filter_and_sort_balls(file_path: str):
    ...
    for row in sheet.iter_rows(min_row=2, values_only=True):
        balls = row[2:7]
        balls = [int(ball) for ball in balls if isinstance(
            ball, (int, float)) or (isinstance(ball, str) and ball.isnumeric())]
        sorted_balls = sorted(balls)
        results.append(sorted_balls)
    frequency_at_index = [[0 for _ in range(5)] for _ in range(81)]
...
def plot_ball_frequencies(ball_frequency_file):
    ...
    positions = [f"position_{i+1}" for i in range(5)]
...
def main():
    input_file = "./resources/quina-history.xlsx"
